...
public class HttpClientFactory {
    ...
    public CloseableHttpClient newClient() {
        ...
        return HttpClients.custom()
                ...
                .addInterceptorFirst(
                        (HttpRequestInterceptor) (request, context) -> urlHolder[0] = ((HttpRequestWrapper) request)
                                .getOriginal()
                                .getRequestLine()
                                .getUri())
                .addInterceptorFirst((HttpResponseInterceptor) (response, context) -> {
                    if (response.getStatusLine().getStatusCode() == HttpStatus.SC_UNAUTHORIZED) {
                        throw new IllegalStateException(
                                "Server returned a 401 status; please check your authentication details for "
                                        + urlHolder[0]);
                    }
                })
                .build();
    ...
    private BasicCredentialsProvider newCredentialsProvider() {
        ...
        credentialsProvider.setCredentials(
                new AuthScope(nexusHost, nexusPort),
                new UsernamePasswordCredentials(asf.getUsername(), asf.getPassword()));
        credentialsProvider.setCredentials(
                new AuthScope("reporter.apache.org", 443),
                new UsernamePasswordCredentials(asf.getUsername(), asf.getPassword()));
        credentialsProvider.setCredentials(
                new AuthScope(jiraHost, jiraPort),
                new UsernamePasswordCredentials(asf.getUsername(), asf.getPassword()));
