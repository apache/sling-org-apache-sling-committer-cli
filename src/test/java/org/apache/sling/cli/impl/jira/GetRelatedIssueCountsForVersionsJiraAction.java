...
public class GetRelatedIssueCountsForVersionsJiraAction implements JiraAction {
    ...
    private static final Pattern VERSION_RELATED_ISSUES =
            Pattern.compile("^/jira/rest/api/2/version/(\\d+)/relatedIssueCounts$");

    @Override
    public boolean tryHandle(HttpExchange ex) throws IOException {
        if (!ex.getRequestMethod().equals("GET")) return false;
