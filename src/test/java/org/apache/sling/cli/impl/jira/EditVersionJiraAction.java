        ...
        try {
            ...
            try (InputStream in = getClass().getResourceAsStream("/jira/versions.json")) {
                ...
                try (InputStreamReader reader = new InputStreamReader(in)) {
                    Gson gson = new Gson();
                    Type collectionType = TypeToken.getParameterized(List.class, Version.class)
                            .getType();
                    List<Version> versions = gson.fromJson(reader, collectionType);
                    Optional<Version> versionHolder = versions.stream()
                            .filter(v -> versionId == v.getId())
                            .findFirst();
                    if (versionHolder.isEmpty()) {
                        ...
                        if ("PUT".equals(ex.getRequestMethod())) {
                            ...
                            try (InputStreamReader requestReader = new InputStreamReader(ex.getRequestBody())) {
                                ...
                                if (versionToUpdate.released != version.isReleased()
                                        && versionToUpdate.released
                                        && dateProvider
                                                .getCurrentDateForJiraRelease()
                                                .equals(versionToUpdate.releaseDate)
